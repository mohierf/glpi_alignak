...
class PluginAlignakCrontasks extends CommonDBTM {
   ...
   static function cronInfo($name){
      switch ($name) {
         case 'genericGraphiteCounters':
            return array ('description' => __('Update generic Graphite counters','alignak'));
            break;

         case 'genericDailyCounters':
            return array ('description' => __('Update generic daily counters','alignak'));
            break;

         case 'specificDailyCounters':
            return array('description' => __('Update specific daily counters', 'alignak'));
            break;

         case 'getCSVDailyCounters':
            return array('description' => __('Send daily counters by mail', 'alignak'));
            break;
      }
      return array();
   }
      ...
      while ($services = $DB->fetch_array($resultServices)) {
         ...
         if (count($columns) > 0) {
            ...
            foreach ($columns as $column) {
               ...
               if (in_array($column['Field'], array ('id','hostname','entities_id','day'))) {
                  continue;
               }
   ...
   static function crongetCSVDailyCounters($notification=0) {
      ...
      foreach ($notifications as $notification) {
         ...
         $notification['to'][] = array ($user->getDefaultEmail(), $user->getName());
         ...
         // Mail copies
         $notification['cc'] = array();
         if ($user->getFromDB($notification['user_cc_1_id'])) {
            $notification['cc'][] = array ($user->getDefaultEmail(), $user->getName());
         }
         ...
         $notification['bcc'] = array();
         if ($user->getFromDB($notification['user_bcc_id'])) {
            $notification['bcc'][] = array ($user->getDefaultEmail(), $user->getName());
         }

         // Components (up to 5 components maximum ...)
         $notification['components'] = array();
...
*/
      ...
      if (count($columns) > 0) {
         ...
         foreach ($columns as $column) {
            // Ignore non counters fields ...
            if (in_array($column['Field'], array ('id','kiosk_name','entities_id','entity_name','day'))) {
               continue;
            }
            $counters[] = $column['Field'];
         }
...
*/
   ...
   static function getCSVDailyCounters($when="previous_week", $notification=null){
      ...
      $headerTitle = array ($enclose.'Jour'.$enclose, $enclose.'Borne'.$enclose);
      $header = array();
      $output = array();
      ...
      foreach ($components as $component) {
         ...
         $result = $DB->query($sql);
         $counters = array( );
         $counters_ratio = array( );
         $counters_decimals = array( );
         while ($data = $DB->fetch_array($result)) {
            foreach ($columns as $column) {
               if ($data['counter_name'] == $column['Field']) {
                  ...
                  $counters_total[$data['counter_name']] = array ("alias" => $data['counter_name'], "title" => $data['name'], "value" => 0, "ratio" => $data['ratio'], "decimals" => $data['decimals']);
