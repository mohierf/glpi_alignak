...
namespace GlpiPlugin\Alignak\Tests;
use Session;
use Html;
use DB;
use Auth;
...
abstract class CommonTestCase extends CommonDBTestCase
{
   ...
   protected function resetGLPILogs() {
      ...
      // Reset plugin log file
      file_put_contents(GLPI_LOG_DIR."/plugin-alignak.log", '');
   }
...
//      Session::start();
   ...
   public function afterTestMethod($method) {
      ...
      $fileLogContent = file_get_contents(GLPI_LOG_DIR."/plugin-alignak.log");
      ...
      if ($fileLogContent != '') {
         rename(GLPI_LOG_DIR."/plugin-alignak.log", GLPI_LOG_DIR."/plugin-alignak__${class}__$method.log");
      }

      // Reset log files
      self::resetGLPILogs();

      // Dump plugin log file
      $this->variable($fileLogContent)->dump($fileLogContent);
